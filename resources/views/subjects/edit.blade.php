...
@section('breadcrumb', 'Modifier la Matière')
...
<div class="px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">Mettre à jour les informations de {{ $subject->name }}</p>
            </div>
    ...
    <div class="rounded-xl bg-white shadow-sm max-w-3xl">
        <form method="POST" action="{{ route('directeur.subjects.update', $subject) }}">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="number" name="coefficient" value="{{ old('coefficient', $subject->coefficient) }}" 
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                              class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                              placeholder="Description de la matière, objectifs, contenu du programme...">{{ old('description', $subject->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="rounded-lg bg-gray-50 border border-gray-200 p-4">
                    <h4 class="text-sm font-semibold text-gray-900 mb-3">Statistiques de la matière</h4>
                    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
                        <div>
                            <p class="text-xs text-gray-500">Notes enregistrées</p>
                            <p class="text-lg font-bold text-gray-900">{{ $subject->grades()->count() }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Étudiants</p>
                            <p class="text-lg font-bold text-gray-900">{{ $subject->class->students()->count() }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Moyenne générale</p>
                            <p class="text-lg font-bold text-gray-900">
                                {{ $subject->grades()->count() > 0 ? number_format($subject->grades()->avg('grade_value'), 2) : 'N/A' }}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-blue-700 mt-1">
                                La modification de l'enseignant ou de la classe peut affecter les notes et présences existantes. Assurez-vous que ces changements sont intentionnels.
                            </p>
                        </div>
                    </div>
                </div>
